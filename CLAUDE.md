...
- **The speaker column is fed, and it is a column** (ADR 0008).
  ...
  it, which is why the slot had never drawn a name. What fills it now is
  `LiveMeetingSession.LiveSegment.speaker`, written once per finalized turn by
  the voiceprint helper (see Live speaker names below).
  `LiveTranscript.blocks` groups consecutive lines by it and the name rides the
  row that **opens** the turn, right-aligned in a column of its own with the
  words left-aligned on one shared edge beside it.
  ...
  *constant* — deliberately unlike the document's, which measures the longest
  ...
  the idiom `Metrics.readingMeasure` uses.

  **Nil is the ordinary state, not a gap.** A name is drawn only for a confident
  match and nothing on screen may be a guess, so the column is empty whenever
  nobody is enrolled, the voice is one nobody has enrolled, the turn was too
  short to embed, or the helper would not start. Not one number in
  `RecordingPaneMetrics` moved when names arrived, which is the whole point of
  reserving it. The volatile tail is a line like any other — it continues the
  ...
  arrives, which pushes the whole transcript down mid-meeting — the movement
