Create Codex pet row `running-right` for `mochi`: exactly 8 full-body frames in one horizontal strip on flat pure magenta #FF00FF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: A compact mint-green plush helper with a rounded body, tiny cream belly patch, expressive dark eyes, short teal ear tufts, and a small yellow satchel; soft felt texture, friendly and readable at pet size.. Preserve silhouette, face, palette, material, proportions, markings, and props.
